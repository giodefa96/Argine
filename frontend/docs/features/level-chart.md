...
- **Chart:** **uPlot** (fast vanilla-JS canvas lib) wrapped in `LevelChart`. uPlot is driven
  imperatively in a `useEffect` and rebuilt when the data changes; timestamps → unix seconds
  on the x axis (`scales.x.time`), level in metres on y.
- **States:** `StationLevel` handles loading / error (`role="alert"`) / empty / data, and
  shows the latest value above the chart.
